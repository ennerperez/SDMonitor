...
- Running the CLI without arguments starts `dashboard` and loads `preferences.json` from the per-user SDMonitor config directory, creating it with defaults when missing.
...
- Osx metrics come from `top`, `sysctl`, `vm_stat`, `ioreg`, `NetworkInterface`, and `DriveInfo`.
- GPU usage uses `nvidia-smi` on Linux/Windows and `ioreg` on Osx when present; otherwise tile shows `N/A`.
