...
public sealed class DashboardSimulationService
{
    ...
    {
        _config = DashboardConfig.Load(null, null);
    ...
    {
        StreamDeckKeyViewModel[] keys = Enumerable.Range(0, MonitorMiniConstants.KeyCount)
        ...
        {
            StreamDeckKeyViewModel key = keys[tile.Position - 1];
    ...
    public DashboardUpdate Update(IReadOnlyList<StreamDeckKeyViewModel> keys)
    {
        ...
        foreach (DashboardTile tile in _config.Tiles)
        {
            ...
            StreamDeckKeyViewModel key = keys[keyIndex];
            ApplyStyle(key, tile.Style);
